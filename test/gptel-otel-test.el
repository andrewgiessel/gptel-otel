...
(ert-deftest gptel-otel-best-practice-names-and-root-payload ()
  (gptel-otel-test--isolated
   (let* ((old-user '(:role "user" :content "old"))
          (assistant '(:role "assistant" :content "reply"))
          (new-user '(:role "user" :content "latest"))
          (info (list :data (list :messages (vector old-user assistant new-user))
                      :model 'model :callback #'ignore))
          (fsm (gptel-otel-test--fsm info)))
     (gptel-otel--instrument-request fsm)
     (gptel-otel--before-wait fsm)
     (let* ((context (gptel-otel--context fsm))
       ...
       (cl-letf (((symbol-function 'gptel-otel-enqueue) (lambda (_request) "queued")))
         ...
         (let* ((input-value (cdr (assoc "langfuse.observation.input"
                                        (gptel-otel-span-attributes root))))
                (input (cdr (assq 'stringValue input-value))))
           (should (string-match-p "latest" input))
           (should-not (string-match-p "old" input))))))))

(ert-deftest gptel-otel-tool-is-sibling-of-requesting-generation ()
  (gptel-otel-test--isolated
   (let* ((call (list :id "tool" :name "Bash" :args '(:command "true")))
          (info (list :data '(:messages []) :model 'model :callback #'ignore
                      :tool-use (list call)))
          (fsm (gptel-otel-test--fsm info)))
     (gptel-otel--instrument-request fsm)
     (gptel-otel--before-wait fsm)
     ...
     (let* ((context (gptel-otel--context fsm))
            (root (gptel-otel--context-root context))
            (tool (gethash call (gptel-otel--context-tool-spans context))))
